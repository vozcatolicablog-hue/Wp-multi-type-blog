...
define( 'WP_MULTIPOST_BLOG_VERSION', '1.1.0' );
 ...
 * @return string
 */
function wp_multipost_blog_sanitize_switch( $settings, $key, $default = 'yes' ) {
 ...
 * @return array
 */
function wp_multipost_blog_build_tax_query( $terms, $relation = 'AND' ) {
	$grouped_terms = array();
 ...
 * @return array
 */
function wp_multipost_blog_allowed_orderby() {
 ...
 * @return array
 */
function wp_multipost_blog_sanitize_settings( $settings ) {
	$settings = is_array( $settings ) ? $settings : array();

	$post_types     = ! empty( $settings['post_types'] ) ? wp_multipost_blog_sanitize_array( (array) $settings['post_types'], 'sanitize_key' ) : array( 'post' );
	$post_types     = wp_multipost_blog_validate_post_types( $post_types );
	$authors        = ! empty( $settings['authors'] ) ? wp_multipost_blog_sanitize_array( (array) $settings['authors'], 'absint' ) : array();
	$terms          = ! empty( $settings['terms'] ) ? wp_multipost_blog_sanitize_array( (array) $settings['terms'], 'sanitize_text_field' ) : array();
	...
	if ( ! empty( $settings['exclude_ids'] ) && is_array( $settings['exclude_ids'] ) ) {
		$exclude_ids = wp_multipost_blog_sanitize_array( $settings['exclude_ids'], 'absint' );
	} else {
		$exclude_ids = ! empty( $settings['exclude_ids'] ) ? wp_multipost_blog_sanitize_array( preg_split( '/[\s,]+/', (string) $settings['exclude_ids'] ), 'absint' ) : array();
	}
	...
	if ( 'yes' === wp_multipost_blog_sanitize_switch( $settings, 'exclude_current_post', 'yes' ) && $current_post_id ) {
	...
	return array(
		...
		'exclude_current_post' => wp_multipost_blog_sanitize_switch( $settings, 'exclude_current_post', 'yes' ),
		'show_featured'        => wp_multipost_blog_sanitize_switch( $settings, 'show_featured', 'yes' ),
		'show_author'          => wp_multipost_blog_sanitize_switch( $settings, 'show_author', 'yes' ),
		'show_date'            => wp_multipost_blog_sanitize_switch( $settings, 'show_date', 'yes' ),
		'show_views'           => wp_multipost_blog_sanitize_switch( $settings, 'show_views', 'yes' ),
		'show_excerpt'         => wp_multipost_blog_sanitize_switch( $settings, 'show_excerpt', 'yes' ),
		'show_category'        => wp_multipost_blog_sanitize_switch( $settings, 'show_category', 'yes' ),
 ...
 * @return string
 */
function wp_multipost_blog_sign_settings( $settings ) {
 ...
 * @return array
 */
function wp_multipost_blog_build_query_args( $settings, $paged = 1 ) {
	...
	if ( ! empty( $settings['terms'] ) ) {
		$tax_query = wp_multipost_blog_build_tax_query( $settings['terms'], $settings['tax_relation'] );
		if ( ! empty( $tax_query ) ) {
 ...
 * @return int
 */
function wp_multipost_blog_get_max_pages( $query, $settings ) {
...
function wp_multipost_blog_load_more_handler() {
	...
	$settings = wp_multipost_blog_sanitize_settings( $settings_raw );
